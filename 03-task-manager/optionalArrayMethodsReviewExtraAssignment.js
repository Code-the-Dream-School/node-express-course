...
const lastNamesSplit = names.map((name) => {
  const lastName = name.split(" ").pop();
  return lastName;
});

console.log(
  "BEGINNING OF ASSIGNMENT HERE!!!!! : Question 1 - ",
  lastNamesSplit
);
...
const wrongFormattingRemoved = names.filter((str) =>
  str.match(/[a-zA-Z]+[\s][a-zA-Z][\w\d]/)
);

console.log("Question 2 - ", wrongFormattingRemoved);
...
const titleCase = names.map((str) => {
  const lowerSplit = str.toLowerCase().split(" ");
  const capsArrayItems = lowerSplit.map((item) => {
    return item.charAt(0).toUpperCase() + item.slice(1);
  });
  return capsArrayItems.join(" ");
});

console.log("Question 3 - ", titleCase);
...
const correctAndCapsArray = wrongFormattingRemoved.map((str) => {
  const lowerSplit = str.toLowerCase().split(" ");
  const capsArrayItems = lowerSplit.map((item) => {
    return item.charAt(0).toUpperCase() + item.slice(1);
  });
...
const lastChallenge = correctAndCapsArray
  .filter((name) => !name.endsWith("z"))
  .map((name) => `${name}, please sign up!`);

console.log("Question 4 - ", lastChallenge);
